...
describe('descarga de artefactos', () => {
  ...
  it('exporta JSON con sangría y tipo de contenido propio', () => {
    ...
    expect(creada.nombre[0]).toMatch(/^niand-calendario-vencimientos-datos-\d{8}-\d{4}\.json$/);
  });
  ...
  it('exporta el calendario con el tipo text/calendar que reconocen Outlook y Google', () => {
    exportarICS('BEGIN:VCALENDAR\r\nEND:VCALENDAR\r\n', 'calendario-2026');

    expect(creada.tipo[0]).toContain('text/calendar');
    expect(creada.nombre[0]).toMatch(
      /^niand-calendario-vencimientos-calendario-2026-\d{8}-\d{4}\.ics$/,
    );
    expect(creada.contenido[0]).toContain('BEGIN:VCALENDAR');
  });
